...
def post_list(request, tag_slug=None):
    ...
    paginator = Paginator(post_list.filter(tags__in=[tag]), '3')
    ...
    return render(request, 'core/post/list.html', {'posts': posts, 'tag': tag})
...
def post_detail(request, year, month, day, post):
    ...
    return render(request, 'core/post/detail.html',
                {'post': post,
                'comments': comments,
                'form': form,
...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_mail(subject, message, 'me@me.com', [cd['to']])
            sent = True
    else:
        form = EmailPostForm()
...
def post_search(request):
    ...
    return render(request, 'core/post/search.html',
                {'form': form,
                'query': query,
                'results': results})